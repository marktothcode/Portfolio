...
router.get("/", function(req, res) {
    ...
    Pillow.find({}, function(err, allPillows) {
        if (err) {
            ...
            res.render("fluffypillows/pillows/index", { pillows: allPillows, currentUser: req.user });
        }
    });
...
router.get("/new", middleware.isLoggedIn, function(req, res) {
    res.render("fluffypillows/pillows/new");
});
...
router.get("/:id", function(req, res) {
    Pillow.findById(req.params.id).populate("comments").exec(function(err, foundPillow) {
        if (err) {
            ...
            res.render("fluffypillows/pillows/show", { pillow: foundPillow });
        }
    });
});
...
router.get("/:id/edit", middleware.checkPillowOwnership, function(req, res) {
    Pillow.findById(req.params.id, function(err, foundPillow) {
        ...
        res.render("fluffypillows/pillows/edit", { pillow: foundPillow });
    });
});
